...
def safe_log(path,req):
    ...
    item={'path':path,'roles':[m.get('role') for m in msgs if isinstance(m,dict)],'tool_names':[t.get('function',{}).get('name') for t in (req.get('tools',[]) or []) if isinstance(t,dict)],'tool_results':[str(m.get('content',''))[-2000:] for m in msgs if isinstance(m,dict) and m.get('role')=='tool']}
...
def final_for(m):return {'tool':'ROT_TOOL_OK','isolation':'ROT_ISOLATION_OK','escape':'ROT_ESCAPE_DONE'}.get(m,'ROT_OK')
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if m!='plain' and not has_tool_result(ms) and name:
