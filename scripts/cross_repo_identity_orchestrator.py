...
def ask(prompt,num_ctx=2048,num_predict=240,schema=None):
    ...
    return api,json.loads(api['response'])
...
def validate_mission(path):
    mission_path=Path(path); m=json.loads(mission_path.read_text())
    assert mission_path.stem==m['mission_id'],(mission_path.stem,m['mission_id'])
...
def canonicalize_test_contract(obj):
    if set(obj)=={'cases'}: chosen=sorted(obj['cases'])
        ...
        assert set(obj)==set(CASES),obj; assert all(obj[k] is True for k in CASES),obj; chosen=sorted(obj)
    assert chosen==CASES,(chosen,CASES); return {'cases':CASES}

def materialize_identity(target: Path):
    p=target/'packages/core/src/identity.ts'; src=p.read_text()
    start=src.index('export function stableSerialize(value: unknown): string {'); end=src.index('\n}\n\n/**\n * 64-bit FNV-1a',start)+2
...
export function stableSerialize(value: unknown): string {
  ...
  switch (typeof value) {
    ...
    case 'object': {
      ...
      const object=value as Record<string, unknown>; const descriptors=Object.getOwnPropertyDescriptors(object);
      const entries=Object.keys(descriptors).map(rawKey=>{ const descriptor=descriptors[rawKey]; if (!descriptor.enumerable || !('value' in descriptor)) throw new Error('Canonical identity object properties must be enumerable data properties'); return {rawKey,normalizedKey:normalizeUnicode(rawKey)}; });
      const normalizedKeys=new Set<string>();
      for (const entry of entries) { if (normalizedKeys.has(entry.normalizedKey)) throw new Error(`Canonical identity key normalization collision: ${entry.normalizedKey}`); normalizedKeys.add(entry.normalizedKey); }
    ...
    replacements={"const value = authority.trim().toLowerCase();":"const value = authority.trim().normalize('NFC').toLowerCase();","const value = resourceType.trim().toLowerCase();":"const value = resourceType.trim().normalize('NFC').toLowerCase();","const value = resourceId.trim();":"const value = resourceId.trim().normalize('NFC');","resourceId: input.resourceId.trim(),":"resourceId: decodeURIComponent(normalizeResourceId(input.resourceId)),"}
    for old,newv in replacements.items(): assert old in src,old; src=src.replace(old,newv)
    p.write_text(src)

def write_tests(target: Path):
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--mission',required=True); ap.add_argument('--orchestrator-commit',required=True); ap.add_argument('--target-dir',default='/tmp/target'); ap.add_argument('--evidence-dir',default='/tmp/cross-repo-evidence'); args=ap.parse_args()
    ...
    baseline_tsc=tsc_errors(target); dump('/tmp/tsc-baseline.json',{'errors':baseline_tsc})
    planner_keys=['use_json_only_domain','normalize_unicode_nfc','reject_unsupported_objects','reject_normalized_key_collisions','defer_provider_case_rules','defer_registry_immutability']; _,planner=ask('You are the PLANNER. Return JSON only with exactly these boolean keys, all true: '+', '.join(planner_keys)+'. No prose.',num_predict=140); assert set(planner)==set(planner_keys) and all(planner[k] is True for k in planner_keys),planner
    expected_plan={'serializer_domain':'JSON_ONLY','unicode_form':'NFC','unsupported_objects':'REJECT','normalized_key_collisions':'REJECT','provider_case_rules':'DEFER','registry_immutability':'DEFER'}; dump('/tmp/planner.contract.json',expected_plan)
    keys=['reject_undefined','reject_bigint','reject_non_plain_objects','reject_symbol_properties','reject_sparse_arrays','reject_array_extra_properties','reject_accessor_properties','normalize_string_values_nfc','normalize_object_keys_nfc','reject_normalized_key_collisions','normalize_identity_fields_nfc']; _,builder=ask('You are the BUILDER. Return JSON only with these keys all boolean true: '+', '.join(keys),num_predict=180); assert set(builder)==set(keys) and all(builder[k] is True for k in keys),builder; dump('/tmp/builder.contract.json',builder); materialize_identity(target)
    _,tester=ask('You are the TEST ENGINEER. Every named adversarial case is required. Return JSON marking each as true: '+', '.join(CASES),num_predict=280); dump('/tmp/tester.contract.json',canonicalize_test_contract(tester)); write_tests(target)
    ...
    run(['npx','--yes','tsx@4.20.5','scripts/test-identity-hardening.ts'],cwd=target,timeout=120); post_tsc=tsc_errors(target); assert post_tsc==baseline_tsc,{'baseline':baseline_tsc,'post':post_tsc}; dump('/tmp/tsc-post.json',{'errors':post_tsc,'no_new_diagnostics':True}); run(['git','diff','--check'],cwd=target)
    ...
    data={'schema':'rotclaw.cross-repo-engineering-evidence.v4','mission_id':m['mission_id'],'mission_file':str(Path(args.mission)),'mission_sha256':h(args.mission),'orchestrator_commit':args.orchestrator_commit,'target_repo':'rotprods/cos-graph-engine','target_source_commit':TARGET_COMMIT,'target_base_ref':m['target_ref'],'changed_paths':ALLOWED,'patch_sha256':h('/tmp/candidate.patch'),'identity_sha256':h(target/ALLOWED[0]),'test_sha256':h(target/ALLOWED[1]),'planner_sha256':h('/tmp/planner.contract.json'),'builder_sha256':h('/tmp/builder.contract.json'),'tester_sha256':h('/tmp/tester.contract.json'),'security_sha256':h('/tmp/security.contract.json'),'reviewer_sha256':h('/tmp/reviewer.contract.json'),'tsc_baseline_sha256':h('/tmp/tsc-baseline.json'),'tsc_post_sha256':h('/tmp/tsc-post.json'),'baseline_type_errors':len(baseline_tsc),'new_type_errors':0,'deterministic_qa':'PASS','authority':'A2_CROSS_REPO_NO_PUSH_NO_MERGE','promotion_authority':'CONTROL_PLANE_ONLY'}; dump(evidence/'EVIDENCE.json',data); print('CROSS_REPO_ENGINEERING_PASS')
