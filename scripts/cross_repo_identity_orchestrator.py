...
def validate_mission(path):
    ...
    assert m['target_commit']==TARGET_COMMIT
    assert sorted(m['allowed_paths'])==sorted(ALLOWED)
    assert set(m['allowed_actions']) <= {'read','edit','test'}
    return m

def canonicalize_test_contract(obj):
    if set(obj)=={'cases'}:
        chosen=sorted(obj['cases'])
    else:
...
export function stableSerialize(value: unknown): string {
  ...
  switch (typeof value) {
    ...
    case 'bigint':
      throw new Error('Unsupported canonical identity input type: bigint');
    case 'object': {
      if (Array.isArray(value)) {
        ...
        if (keys.length !== value.length || keys.some((key, index) => key !== String(index))) {
          throw new Error('Canonical identity arrays must be dense and must not contain extra properties');
        }
        for (let index = 0; index < value.length; index += 1) {
          const descriptor = Object.getOwnPropertyDescriptor(value, String(index));
          if (!descriptor || !descriptor.enumerable || !('value' in descriptor)) {
            throw new Error('Canonical identity array entries must be enumerable data properties');
          }
        }
      ...
      const entries = Object.keys(descriptors).map(rawKey => {
        ...
        if (!descriptor.enumerable || !('value' in descriptor)) {
          throw new Error('Canonical identity object properties must be enumerable data properties');
        }
...
assert.throws(() => stableSerialize(undefined), /unsupported/i);
assert.throws(() => stableSerialize(1n), /unsupported/i);
assert.throws(() => stableSerialize(new Date()), /prototype/i);
assert.throws(() => stableSerialize(new Map()), /prototype/i);
assert.throws(() => stableSerialize(new Set()), /prototype/i);
assert.throws(() => stableSerialize(new Uint8Array([1, 2])), /prototype/i);
class Box { value = 1; }
assert.throws(() => stableSerialize(new Box()), /prototype/i);
const withSymbol = { value: 1 } as Record<PropertyKey, unknown>;
withSymbol[Symbol('hidden')] = 2;
assert.throws(() => stableSerialize(withSymbol), /symbol/i);
const sparse = new Array(2); sparse[1] = 'x';
assert.throws(() => stableSerialize(sparse), /dense/i);
const extra = ['x'] as string[] & { extra?: string }; extra.extra = 'y';
assert.throws(() => stableSerialize(extra), /extra properties/i);
const accessor = {} as Record<string, unknown>;
Object.defineProperty(accessor, 'x', { enumerable: true, get: () => 1 });
assert.throws(() => stableSerialize(accessor), /data properties/i);
const nullProto = Object.create(null) as Record<string, unknown>;
nullProto.b = 2; nullProto.a = 1;
assert.equal(stableSerialize(nullProto), '{"a":1,"b":2}');
const uriA = canonicalUri({ scheme: 'github', authority: 'GitHub.COM', resourceType: 'Repo', resourceId: composed });
const uriB = canonicalUri({ scheme: 'github', authority: 'github.com', resourceType: 'repo', resourceId: decomposed });
...
def main():
    ...
    dump('/tmp/planner.raw-contract.json',planner)
    ...
    security_schema={'type':'object','additionalProperties':False,'properties':{'verdict':{'type':'string','enum':['PASS','FAIL']},'defect_code':{'type':'string','enum':['NONE','AMBIGUOUS_CANONICALIZATION','EXECUTABLE_PROPERTY','UNSUPPORTED_OBJECT_ACCEPTED','COLLISION_GAP','OTHER']},'finding':{'type':'string','maxLength':180}},'required':['verdict','defect_code','finding']}
    _,security=ask('You are SECURITY REVIEWER. Deterministic tests passed. Review canonical ambiguity and executable-property risks. PASS only if no concrete defect. PATCH:\n'+patch,4096,180,security_schema)
    assert security['verdict']=='PASS' and security['defect_code']=='NONE',security; dump('/tmp/security.contract.json',security)
    reviewer_schema={'type':'object','additionalProperties':False,'properties':{'verdict':{'type':'string','enum':['PASS','FAIL']},'defect_code':{'type':'string','enum':['NONE','ARRAY_SEMANTICS','OBJECT_SEMANTICS','UNICODE_SEMANTICS','URI_SEMANTICS','SCOPE_CREEP','OTHER']},'finding':{'type':'string','maxLength':180}},'required':['verdict','defect_code','finding']}
    _,reviewer=ask('You are independent CODE REVIEWER. Intended breaking change: undefined/bigint reject. Find any other concrete regression. PASS only if none. PATCH:\n'+patch,4096,180,reviewer_schema)
    assert reviewer['verdict']=='PASS' and reviewer['defect_code']=='NONE',reviewer; dump('/tmp/reviewer.contract.json',reviewer)
