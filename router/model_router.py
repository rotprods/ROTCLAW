...
CANONICAL={
 "glm-5.2":os.environ.get("ROT_MODEL_GLM","glm-5.2"),
 "minimax-m3":os.environ.get("ROT_MODEL_MINIMAX","minimax-m3"),
 "kimi-k2.6":os.environ.get("ROT_MODEL_KIMI","kimi-k2.6"),
 "deepseek-v4-flash":os.environ.get("ROT_MODEL_DEEPSEEK","deepseek-v4-flash"),
}
ALIASES={
...
def canonical(m): return ALIASES.get(m,PROFILES.get(m,m))
def resolve(m):
 c=canonical(m)
...
class H(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  if self.path=="/healthz": return self.sendj(200,{"ok":True,"upstream":UPSTREAM,"auth_configured":bool(TOKEN),"api_key_configured":bool(API_KEY),"canonical_models":sorted(CANONICAL)})
  if not self.auth(): return self.sendj(401,{"error":"unauthorized"})
  if self.path=="/v1/models": return self.sendj(200,{"object":"list","data":[{"id":x,"object":"model"} for x in sorted(CANONICAL)]})
